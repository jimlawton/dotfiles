fish_add_path --global --path $HOME/bin $HOME/.cargo/bin $HOME/go/bin $HOME/.docker/bin
fish_add_path --global --path $HOMEBREW_PREFIX/bin $HOMEBREW_PREFIX/sbin
fish_add_path --global --path /usr/local/sbin
fish_add_path --global --path /opt/homebrew/opt/libpq/bin
fish_add_path --global --path /opt/homebrew/opt/gnu-sed/libexec/gnubin
fish_add_path --global --path /opt/homebrew/opt/make/libexec/gnubin
fish_add_path --global --path /opt/homebrew/opt/llvm/bin
fish_add_path --global --path /Applications/Obsidian.app/Contents/MacOS

